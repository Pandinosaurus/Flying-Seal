...
class Game:
	...
	def initScreen(self):
		#screen is an abstraction for the display : pygame.display
		#but is screen is a surface! 
		#Modifying screen will modify what you want to display
		#but to show it, you should call pygame.display.flip() or update()
		self.SCREENWIDTH = 300
		self.SCREENHEIGHT = 400
		self.SCREEN = pygame.display.set_mode((self.SCREENWIDTH, self.SCREENHEIGHT), 
											  HWSURFACE | DOUBLEBUF | RESIZABLE)
		self.MAINFONT = pygame.font.SysFont('Arial',16, True)
		self.TITLEFONT = pygame.font.SysFont('Arial',24, True)
	...
	def initText(self):
		self.setTitleText('Flying Seal', cyan)
	...
	def setPlayers(self):
		...
		self.PLAYERJUMPVELOCITY = 12
		self.PLAYERGRAVITY = 0.71
	...
	def titleScreen(self):
		...
		logo = pygame.transform.scale(self.icon, self.PLAYERSIZE)
		...
		self.SCREEN.blit(logo, (self.SCREENWIDTH/2  - logo.get_width()/2,
		...
		while not done:
			for event in pygame.event.get():
				...
				if event.type == pygame.KEYDOWN:
					...
					elif key != pygame.K_BACKSPACE and key != pygame.K_SPACE and key!= pygame.K_UP and key != pygame.K_DOWN and key!= pygame.K_LEFT and key != pygame.K_RIGHT:
						if   len(self.USERNAME) > 0:
